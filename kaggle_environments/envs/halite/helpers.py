...
class Configuration(kaggle_environments.helpers.Configuration):
    ...
    @property
    def numberPirates(self) -> int:
        """The seed to the random number generator (0 means no seed)."""
        return self["numberPirates"]
...
class Player:
    def __init__(self, player_id: PlayerId, halite: int, shipyard_ids: List[ShipyardId], ship_ids: List[ShipId], board: 'Board', is_pirate: bool) -> None:
        self._id = player_id
        self._halite = halite
        self._shipyard_ids = shipyard_ids
        self._ship_ids = ship_ids
        self._board = board
        self._is_pirate = is_pirate
    ...
    def _observation(self):
        ...
        return [self.halite, shipyards, ships, self.is_pirate]
# endregion
...
class Board:
    def __init__(
        ...
        for (player_id, player_observation) in enumerate(observation.players):
            ...
            [player_halite, player_shipyards, player_ships, is_pirate] = player_observation
            # We'll populate the player's ships and shipyards in _add_ship and _add_shipyard
            self.players[player_id] = Player(player_id, player_halite, [], [], self, is_pirate)
        ...
        for player in board.players.values():
            ...
            for ship in player.ships:
                if ship.next_action == ShipAction.CONVERT:
                    if ship.cell.shipyard_id is None:
                        if player.is_pirate:
                        ...
                        elif (ship.halite + player.halite) >= convert_cost:
