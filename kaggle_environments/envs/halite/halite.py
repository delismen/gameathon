...
def pirate(board):
    ...
    for ship in ships:
        if ship.cell.shipyard is None:
            if len(shipyards) == 0 and not converted:
                ship.next_action = ShipAction.CONVERT
                converted = True
                continue

        neigbours = [
            ship.cell.north.shipyard_id,
            ship.cell.east.shipyard_id,
            ship.cell.south.shipyard_id,
            ship.cell.west.shipyard_id
        ]
        possible_moves = [ShipAction.moves()[i] for i, x in enumerate(neigbours) if x is None]
        if possible_moves:
            ship.next_action = choice(possible_moves)
        else:
            ship.next_action = choice(ShipAction.moves())

    for shipyard in shipyards:
        if len(ships) < 3 and not spawned:
            shipyard.next_action = ShipyardAction.SPAWN
            spawned = True


agents = {"random": random_agent, "pirate": pirate}
...
def populate_board(state, env):
    ...
    obs.players = []
    pirates_num = config.numberPirates
    players_num = num_agents - pirates_num

    if pirates_num == 1:
        pirate_positions = [starting_positions[randint(0,3)]]
    elif pirates_num == 2:
        pirate_positions = choice([
            [starting_positions[0], starting_positions[3]],
            [starting_positions[1], starting_positions[2]]
        ])
    else:
        pirate_positions = []
    agent_positions = [x for x in starting_positions if x not in pirate_positions]

    for i in range(num_agents):
        if i < players_num:
            ships = {create_uid(): [agent_positions[i], 0]}
            obs.players.append([state[0].reward, {}, ships, False])
        else:
            ships = {create_uid(): [pirate_positions[i - players_num], 0]}
            obs.players.append([0, {}, ships, True])
...
def interpreter(state, env):
    ...
    for index, agent in enumerate(state):
        player_halite, shipyards, ships, is_pirate  = obs.players[index]
        if not is_pirate and agent.status == "ACTIVE" and len(ships) == 0 and (len(shipyards) == 0 or player_halite < config.spawnCost):
        ...
        if agent.status != "ACTIVE" and agent.status != "DONE":
            obs.players[index] = [0, {}, {}, is_pirate]

    # Check if done (< 2 players and num_agents > 1)
    if len(state) > 1 and sum(1 for agent in state if agent.status == "ACTIVE") - config.numberPirates < 2:
        for agent in state:
